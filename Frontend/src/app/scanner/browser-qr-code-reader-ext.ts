import Reader from './zxing-typescript/core/Reader';
import BinaryBitmap from './zxing-typescript/core/BinaryBitmap';
import HybridBinarizer from './zxing-typescript/core/common/HybridBinarizer';
import Result from './zxing-typescript/core/Result';
import Exception from './zxing-typescript/core/Exception';
import HTMLCanvasElementLuminanceSource from './zxing-typescript/browser/HTMLCanvasElementLuminanceSource';
import QRCodeReader from './zxing-typescript/core/qrcode/QRCodeReader';
...
export class BrowserCodeReaderExt {
  ...
  private decode(callbackFn: (result: Result) =>
    any, retryIfNotFound: boolean = true, retryIfChecksumOrFormatError: boolean = true, once = false): void {
    ...
    try {
      ...
      if (retryIfNotFound && Exception.isOfType(re, Exception.NotFoundException)) {
        ...
        (Exception.isOfType(re, Exception.ChecksumException) || Exception.isOfType(re, Exception.FormatException))) {
        this.decodeWithDelay(callbackFn);
      }
    }
  }
