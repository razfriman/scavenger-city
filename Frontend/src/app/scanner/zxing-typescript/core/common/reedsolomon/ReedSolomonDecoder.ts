...
export default class ReedSolomonDecoder {
  ...
  public decode(received: Int32Array, twoS: number /*int*/): void /*throws ReedSolomonException*/ {
    ...
    const sigmaOmega = this.runEuclideanAlgorithm(GenericGFPoly.buildMonomial(field, twoS, 1), syndrome, twoS);
  ...
  private runEuclideanAlgorithm(a: GenericGFPoly, b: GenericGFPoly, R: number /*int*/): GenericGFPoly[] {
    ...
    let rLast = a;
    let r = b;
    let tLast = GenericGFPoly.getZero(field);
    let t = GenericGFPoly.getOne(field);
    ...
    while (r.getDegree() >= R / 2) {
      ...
      r = rLastLast;
      let q = GenericGFPoly.getZero(field);
      ...
      while (r.getDegree() >= rLast.getDegree() && !r.isZero()) {
        ...
        q = q.addOrSubtract(GenericGFPoly.buildMonomial(field, degreeDiff, scale));
